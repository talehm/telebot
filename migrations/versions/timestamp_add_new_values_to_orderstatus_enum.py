...
def upgrade():
    ...
    op.execute(
        f"ALTER TYPE orderstatus ADD VALUE 'ORDER_SCREENSHOT_SENT' BEFORE 'COMPLETED'"
    )
    op.execute(
        f"ALTER TYPE orderstatus ADD VALUE 'ORDER_WAITING_SCREENSHOT' BEFORE 'COMPLETED'"
    )
    op.execute(
        f"ALTER TYPE orderstatus ADD VALUE 'ORDER_SCREENSHOT_ACCEPTED' BEFORE 'COMPLETED'"
    )
    op.execute(
        f"ALTER TYPE orderstatus ADD VALUE 'ORDER_SCREENSHOT_REJECTED' BEFORE 'COMPLETED'"
    )
