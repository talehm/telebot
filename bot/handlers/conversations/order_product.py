...
def is_buyer_data_expired(context):
    ...
    return helpers.is_older_than(buyer.updated_at, weeks = constraints.BUYER_DATA_EXPIRATION)
...
def save_buyer_info(update, context, column, text = None):
    try:
        chat_id = context.user_data.get("chat_id")
        if not chat_id:
            update.message.reply_text("Chat ID not found. Please start the conversation again.")
            return
        
        buyer = dbHelper.get_one(models.Buyer, chat_id = chat_id)
        if not buyer:
            update.message.reply_text("Buyer not found. Please start the conversation again.")
            return
...
def send_to_seller(update, context, action = None):
    try:
        ...
        if action is constants.NEW_ORDER_REQUEST:
            ...
            # 
            message = f"🔔 <b>New Order Request Received.</b> 🔔\n\n"\
            f"🆔 {product.id}\n"\
            ...
            context.bot.sendMessage(chat_id = seller.chat_id, text = message,  parse_mode = ParseMode.HTML)
            context.bot.send_photo(chat_id = seller.chat_id, photo=open(buyer.amazon_screenshot, 'rb'), caption="Amazon Account Screenshot")
            keyboard = [
                [
                    InlineKeyboardButton("✅ Confirm", callback_data = json.dumps({"action":"confirm_new_order_request", "order_id":order.id})),
                    InlineKeyboardButton("❌ Reject", callback_data=json.dumps({"action":"reject_new_order_request", "order_id":order.id})),
                ]
            ]
            ...
            context.bot.send_photo(chat_id = seller.chat_id, photo=open(context.user_data['amazon_order_screenshot'], 'rb'), caption = message, parse_mode=ParseMode.MARKDOWN_V2)
            # 
            ...
            keyboard = [
                [
                    InlineKeyboardButton("✅ Confirm", callback_data = json.dumps({"action":"accept_order_screenshot", "order_id":order.id})),
                    InlineKeyboardButton("❌ Reject", callback_data = json.dumps({"action":"decline_order_screenshot", "order_id":order.id})),
                ]
            ]
            
        reply_markup = InlineKeyboardMarkup(keyboard)
        context.bot.sendMessage(chat_id = seller.chat_id, text="Please choose an option:", reply_markup = reply_markup)
    
    except Exception as e:  
        update.message.reply_text(f"Error occured. Message is not delivered to seller:{e}")
...
def validate_product_id(update, context):
    ...
    if not text.isdigit():
        ...
        if not product: 
            ...
            buyer = dbHelper.get_one(models.Buyer, chat_id = context.user_data.get("chat_id"))
            order = dbHelper.get_one(models.Order, product_id=product.id, buyer_id=buyer.id)
            # if order is not None:
            #     update.message.reply_text(f"You have already proceeded with this order. Status: {order.status}")
            if isExpired:
                update.message.reply_text("*Next Step: Please send your paypal account.*")
                context.user_data['next_step'] = 'get_paypal_account'
                return STATE2
            elif has_amazon_ss is False:
                update.message.reply_text("*Next Step: Please send your screenshot of amazon profile.*")
...
def get_amazon_account_screenshot(update, context):
    path = save_photo(update, context, constants.ACCOUNT_SCREENSHOT)
    if path:
        isSaved = save_buyer_info(update, context, "amazon_screenshot", path)
        if isSaved:
            context.user_data['amazon_screenshot'] = path
            send_to_seller(update, context, constants.NEW_ORDER_REQUEST)
            update.message.reply_text("Data is sent to seller. Please stay tuned until confirmation")
...
def send_order_screenshot_to_seller(update, context):
    order = dbHelper.get_one(models.Order, id = context.user_data["order_id"])
    if order.status is enums.OrderStatus.SS_REJECTED or order.status is enums.OrderStatus.WAITING_SS:
        path = save_photo(update, context, constants.ORDER_SCREENSHOT)
        if path:
            ...
            update.message.reply_text("Screenshot is sent to seller. Please stay tuned until confirmation")
            return STATE6
    else:
        return STATE7
    update.message.reply_text("Photo is not saved. Please try again") 
...
states = {
    STATE1: [MessageHandler(Filters.text & ~Filters.command, save_message(validate_product_id))],
    STATE2: [MessageHandler(Filters.text & ~Filters.command, save_message(get_paypal_account))],
    STATE3: [MessageHandler(Filters.text & ~Filters.command, save_message(get_amazon_account_link))],
    STATE4: [MessageHandler(Filters.photo, save_message(get_amazon_account_screenshot))],
    STATE5: [MessageHandler(Filters.text & ~Filters.command, save_message(send_to_seller))],
    STATE6: [MessageHandler(Filters.photo, save_message(send_order_screenshot_to_seller))],
